...
def test_profile_range_is_noop_when_disabled():
    ...
    with patch.object(moe_profile, "_MOE_PROFILE_RANGES_ENABLED", False):
        context = moe_profile.moe_profile_range("catccos", "kernel", hidden_states, topk_ids)
...
def test_profile_range_contains_backend_stage_and_real_shape():
    ...
    with (
        ...
        moe_profile.moe_profile_range("native_mc2", "dispatch", hidden_states, topk_ids)

    record_function.assert_called_once_with("vllm_ascend.moe.native_mc2.dispatch[M=138,H=2048,topK=8]")
