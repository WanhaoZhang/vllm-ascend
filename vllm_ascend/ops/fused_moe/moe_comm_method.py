...
from vllm_ascend.ops.fused_moe.catccos_adapter import CatccosLayerCapability, apply_catccos, catccos_backend_enabled
...
def setup_moe_comm_method(moe_config, catccos_capability: CatccosLayerCapability | None = None):
    if moe_config.ep_size > 1:
        _MoECommMethods[MoECommType.ALLTOALL] = AlltoAllCommImpl(moe_config)
        _MoECommMethods[MoECommType.ALLGATHER] = AllGatherCommImpl(moe_config)
        _MoECommMethods[MoECommType.MC2] = MC2CommImpl(moe_config)
        _MoECommMethods[MoECommType.FUSED_MC2] = FusedMC2CommImpl(
            moe_config, catccos_capability=catccos_capability
        )
    else:
        _MoECommMethods[MoECommType.ALLGATHER] = AllGatherCommImpl(moe_config)
...
class FusedMC2CommImpl(MoECommMethod):
    ...
    def __init__(self, moe_config, *, catccos_capability: CatccosLayerCapability | None = None):
